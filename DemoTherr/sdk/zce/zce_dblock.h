...
public:
    ...
    zce_dblock(size_t size, zce_byte* buf); //assign buffer, buffer managed outside
...
public:
    ...
    void reset(int preserved);

    int copy(const zce_byte* data, size_t len);

    void crunch(int preserved = 0);
    ...
    void resize(unsigned newsize); //won't crunch in case of protocol stack reserve
};
