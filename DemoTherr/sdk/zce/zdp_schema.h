...
namespace zdp
{
    ...
    struct ZCE_API zdp_head
    {
        ...
        inline zce_uint32 len() const { return rev() == 3 ? GEP_HEADLEN : ZDP_HEADLEN; }
		inline void set_rev(zce_byte v) { attrib = (attrib & 0xF) | (v << 4); }
		inline void set_zip(zce_byte v) { attrib = (attrib & 0xFC) | (v & 0x3); }
    };

    static inline unsigned zdp_msglen(zce_byte* buf) //should ensure >= 10
    {
        if (buf[0] != 'G')
    ...
    int ZCE_API zdp_unpack(zdp_head&, const zce_byte* buf, size_t size, zce_byte rev);

    int ZCE_API zdp_pack(const zdp_head&, zce_byte* buf, size_t size, zce_byte rev);

    int ZCE_API zdp_reset_seq(const zdp_head&, zce_byte* buf, size_t size, zce_byte rev);

    int ZCE_API zdp_reset_zip(zce_byte v, zce_byte* buf, size_t size, zce_byte rev);
