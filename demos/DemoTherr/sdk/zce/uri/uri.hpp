...
/**
 * \ingroup uri
 * \class uri_builder zce/uri/uri_builder.hpp zce/uri.hpp
...
/**
 * \ingroup uri
 * \class uri zce/uri/uri.hpp zce/uri.hpp
 ...
 *
 * \code
 * zce::uri instance("http://cpp-netlib.org/");
...
class uri {
 ...
 public:
   ...
   */
  typedef zce::string_view string_view;
...
namespace std {
template <>
struct is_error_code_enum<zce::uri_error> : public true_type {};
}  // namespace std

namespace std {
template <>
struct hash<zce::uri> {
  std::size_t operator()(const zce::uri &uri_) const {
    std::size_t seed = 0;
    std::for_each(std::begin(uri_), std::end(uri_),
                  [&seed](zce::uri::value_type v) {
                    std::hash<zce::uri::value_type> hasher;
...
#include <zce/uri/uri_builder.hpp>
