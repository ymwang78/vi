...
@protocol FlyMovieGLRenderer
- (BOOL) isValid;
- (NSString *) fragmentShader;
- (void) resolveUniforms: (GLuint) program;
- (void) setFrame: (unsigned char *)pImage nWidth:(int)width nHeight:(int)height;
- (BOOL) prepareRender;
@end

@interface FlyMovieGLRenderer_RGB : NSObject<FlyMovieGLRenderer> {
...
@implementation FlyMovieGLRenderer_RGB
...
@interface FlyMovieGLRenderer_YUV : NSObject<FlyMovieGLRenderer> {
...
@implementation FlyMovieGLRenderer_YUV
...
- (void) setFrame: (UInt8 *)pImage nWidth:(int)width nHeight:(int)height
{
    
    const NSUInteger frameWidth = width;
    const NSUInteger frameHeight = height;
    ...
    const UInt8 *pixels[3] = { pImage, pImage + width * height, pImage + width * height * 5 / 4 };
...
@implementation FlyGLView {
    ...
    int             _srcWidth;
    int             _srcHeight;
    FlyPixelFormat  _pixelFormat;
    
    id<FlyMovieGLRenderer> _renderer;
}
             ...
             videoPixelFormat:(FlyPixelFormat)format
{
    ...
    if (self) {
        ...
        if (_pixelFormat == FlyPixelFormatYUV) {
            
            _renderer = [[FlyMovieGLRenderer_YUV alloc] init];
            NSLog(@"OK use YUV GL renderer");
            
        } else {
            
            _renderer = [[FlyMovieGLRenderer_RGB alloc] init];
...
- (void) resetSrcVideoSize:(int)width videoHeight:(int)height
{
    ...
    if (_renderer.isValid)
        [self render:nil nWidth:0 nHeight:0];
}

- (void)dealloc
{
    NSLog(@"FlyView dealloc");
...
- (void)updateVertices
{
    ...
    const float width   = _srcWidth;
    const float height  = _srcHeight;
...
{        
    ...
    if (pImage) {
        [_renderer setFrame:pImage nWidth:width nHeight:height];
    }
